...
export default function TelefericosScreen() {
  const { categories } = useRoutes()
  const [selectedRoutes, setSelectedRoutes] = useState<string[]>([])
  const [showRoutes, setShowRoutes] = useState(true)
  const fadeAnim = React.useRef(new Animated.Value(1)).current

  const telefericoCategory = categories.find(cat => cat.id === "telefericos")
  const routes = telefericoCategory?.routes || []
      ...
      <View style={styles.floatingControls}>
        <View style={styles.routesPanel}>
          <View style={styles.panelHeader}>
            <Text style={styles.panelTitle}>Teleféricos - Mi Teleférico</Text>
            <TouchableOpacity
